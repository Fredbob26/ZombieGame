        ...
        public class Kit_PvE_ZombieWaveSurvival_ZombieAI : MonoBehaviourPunCallbacks, IPunObservable
        {
            ...
            private void Update()
            {
                if (photonView.IsMine)
                {
                    ...
                    {
                        //Random voice
                        if (activeRenderer.settings.soundRandom.Length > 0 && activeRenderer.voiceSource)
                        {
                            //Check random ?
                            if (Random.Range(0, 100) >= 100 - activeRenderer.settings.soundRandomChance)
                            {
                                //Get sound
                                int sound = Random.Range(0, activeRenderer.settings.soundRandom.Length);

                                //Send RPC
                                photonView.RPC("RpcPlayVoice", RpcTarget.All, (byte)2, sound);
                            }
                        }
